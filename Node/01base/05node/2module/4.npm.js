...
// 全局包仅仅在命令中使用， 做一些工具使用，如：
// nrm 切换源  npm、nrm(node regisiter manager)
// nvm （node version manager  nvm-win）
// nrm use cnpm/taobao/npm 某个源  npm install 

// 本地模块，有两种
//  开发依赖 --save-dev（-D）（仅在开发的时候使用 gulp） 
// 项目依赖  --save （-S）（开发上线都需要 vue react）
// 也可以不区分 没有很大影响
...
// npx运行命令，如果模板不存在会下载然后执行 -》 之后再删除；如果当先项目下有可以直接复用，如果没有会先安装 (好处就是最新的)
// 项目中功能复杂，执行的命令都很长， 使用npx执行没法保存参数  （临时用完就销毁 包、环境变量都没有了）