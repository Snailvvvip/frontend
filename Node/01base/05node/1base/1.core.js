...
// windows下 可以使用 set 命令来设置  
// mac下 export 
// -》 cross-env包：根据不同的环境(终端)设置环境变量，设置的是临时变量，窗口关掉就消失了
let requestUrl = ''
...
// argv：执行命令时所带的参数，打印process.argv输出值时有两个默认的值
// 1.代表的是可执行node.exe 
// 2.执行的是哪个文件
...
//     return memo;
// },{})
// console.log(userArgs) // { prot: '3000', config: '.env' }


// commander 命令行管家，是第三方模块，用的时候需要下载 
...
// requestFrameAnimation是渲染之前调用的方法，比promise.then慢，可以说是宏任务存在争议
// nextTick ：异步的，把他叫成微任务。不属于事件环的一部分， 下一队列，本轮任务执行完毕后，会立即执行他的回调函数
process.nextTick(()=>{ 
...
// node和浏览器事件环的差异是： 浏览器1个队列  node多个队列
// 执行顺序都是 先执行代码 => （node会清空nextTick） =>  清空微任务 => 再去取出一个宏任务来执行， 当到达poll阶段后，如果check阶段为空，而且poll里面也没有io操作，此时，不会继续轮训，会等待定时器到达时间重新执行,或者有新的i/o操作进入到poll阶段中
