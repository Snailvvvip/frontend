...
console.log(path.resolve('a', 'b', 'c', '..')); // 他也具备拼接的功能，但是最终出来的结果是一个绝对路径,
// path.resolve 遇到 / 表示的是根路径， 默认以当前路径（process.cwd()）解析成绝对路径
// join和resolve区别主要看有没有 / ，其他情况下用谁都写
...
// 让一个字符串执行的方法：1.eval 2.new Function
// eval('console.log(fs)')
// eval 执行的时候会采用上级的变量 
// new Function 创建的函数只能在全局作用域中运行
// global.c = 100;
// const sum = new Function('a', 'b', 'return a + b + c');
// 这两个方法可能会受到外界因素影响, 不安全
