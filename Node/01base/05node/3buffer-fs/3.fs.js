...
// 不适合大文件读取，如果是简单的文件可以使用上面这种方式。 
// 对于大文件来说我们操作全部使用“流” 。
// 流的特点就是有方向，从一个地方到另一个地方

// 底层的文件操作，需要对文件进行精确的读取 
// fs.open fs.read fs.open fs.write (开发的时候用不到，通过这个些api掌握底层原理实现，比如stream的实现原理pipe)
...
// params: path, flags, mode(八进制读写权限), callback
//mode: 读 写 和执行 分成三部分 三个数来标识  1 执行 4读取 2代表写入  进制组合 (权限...)
// chmod -R 777
fs.open(path.resolve(__dirname, 'note.md'), 'r', 0o666, function(err, fd) { // fd:文件描述符
    fs.open(path.resolve(__dirname, 'copy.md'), 'w', function(err, wfd) {
        ...
        function next() {
            // buf 写入到哪个buffer中，从buffer的哪个位置(offset偏移量)开始写入3(length)个字节，从文件的第0(position)个位置开始读取
            fs.read(fd, buf, 0, 3, 0, function(err, bytesRead) { // bytesRead 实际读取到的个数,是字节类型，这个数不一定是3，可能会小于3(取决于文件中的个数)
