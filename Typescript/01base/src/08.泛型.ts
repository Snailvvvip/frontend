...
interface IArgs {
    [x:string]: any // key的类型只能是string 或者number
}
let args:IArgs = {
    a:1,
    1:'x',
...
let arr0:IArr<string> = ['a','b']; // 限制key只能是number类型
...
// 类只能描述实例，但是我们现在要描述类的本身，可以通过下面两种方式
// 1. typeof Cat 可以取出这个类的类型  (在ts中的typeof 只能用去取已有的类型的类型)
// 2. new (name: string, age: number) => Cat 表示是一个构造函数 ，我们限制返回的实例
...
// }
// let forEach = <T>(arr: T[], fn: IFn) => { // 兼容性
//     for (let i = 0; i < arr.length; i++) {
//         fn<T>(arr[i], i); // 执行的时候确定类型
//     }
// };
