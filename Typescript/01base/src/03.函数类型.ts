...
// 函数 我们一般也会标识类型 包括 函数的参数 和 函数的返回值
// 函数声明类型：1.function来声明    2.表达式来声明
// 区别：表达式声明可以给函数本身增加类型，function声明无法给本身声明类型

// ************* 表达式来声明 ***************
// 如果能自动推断出来的我们可以不用标识类型
let sum0 = function (a: string, b: string) {
    return a + b;
}
...
// ************* function来声明 ***************
// function函数只能标识参数类型和返回值类型，无法给函数本身增加类型
function sum1(a: string, b: string): string {
    return a + b
}

// 函数的参数可以做一系列的处理：可选参数，默认值，剩余运算符
...
function spread(...args: number[]) { // 剩余运算符
...
let arr = toArray('123') // 推断arr的返回值类型，只写toArray(val: string | number) 则推断不准确推断出，类型范围扩大(string | number)了
let item = arr[0];
