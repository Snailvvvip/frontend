...
// 类中的概念  实例的属性  实例共享属性  静态属性/方法(类来调用的)
...
class Pointer{
    ...
    constructor(x:number,y?:number){ // 构造函数和普通函数的参数一样可以支持可选参数、 默认参数 、剩余参数
        this.x = x; // 如果只添加实例类型而没有赋值，ts默认不能将null 赋值给其他类型
...
// 类的修饰符 java（public、protected、private、readonly）  限制访问的范围 
...
class Cat extends Animal{ // Cat.__proto__ = Animal
    // private name:string = ''
    constructor(type:string,private name:string){ //接收name 并直接声明到实例上 !!!!
        super(type);
        // this.name = name;
    }
    ...
    get newName(){
        return this.name // 访问constructor的参数name
    }
    set newName(newVal){ // 校验 和 属性的保护，防止非法篡改name
        this.name = newVal;
    }
    eat(){ // 子类重写的原型方法 要和父类的一致
        super.eat(); // super指代的是父类的原型
        console.log('猫 eat')
        return ''
    }
}
